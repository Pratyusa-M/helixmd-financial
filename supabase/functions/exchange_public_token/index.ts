...
const PLAID_URLS: { [key: string]: string } = {
...
serve(async (req: Request) => {
  ...
  const SUPABASE_SERVICE_KEY = Deno.env.get("SUPABASE_SERVICE_KEY"); // Set this in dashboard, service role key for writes
  ...
  if (!baseUrl) {
    return new Response("Invalid Plaid environment", { status: 400 });
  }
  ...
  if (!public_token || !metadata) {
    return new Response("Missing public_token or metadata", { status: 400 });
  }
  ...
  try {
    ...
    const { error } = await supabaseAdmin.from("connected_accounts").insert({
      ...
      account_type: metadata.accounts[0].type, // Assuming first account
      status: "active",
    ...
    console.error(error);
    return new Response(JSON.stringify({ error: "Failed to exchange token or store account" }), { status: 500 });
  }
});