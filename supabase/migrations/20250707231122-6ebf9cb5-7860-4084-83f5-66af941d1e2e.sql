...
-- Recreate the income transactions view with proper enum casting
CREATE OR REPLACE VIEW public.view_income_transactions AS
SELECT *
FROM public.transactions
WHERE 
  direction = 'credit'::public.transaction_direction
...
CREATE INDEX idx_transactions_income_filter ON public.transactions(direction, account_type, amount) WHERE direction = 'credit'::public.transaction_direction AND account_type IN ('chequing', 'savings');